<?php
session_start();
ob_start();
include('expire.inc.php');
include('cat.inc.php');

$numRows=0;
$conn = new mysqli($host,$user,$pwd,$db) or die("Can't connect");

$message  = '';
$email='';
$fax='';
$phone='';
$clientId = $_SESSION['clientID'];




$sql="select email,fname,lname, fax,phone from client where client_id =$clientId";

$result = $conn->query($sql);


		
	


if(isset($_POST['submit'])){

    

	$phone = strip_tags($_POST['txt_phone']);
	$fax = strip_tags($_POST['txt_fax']);
	$fname = strip_tags($_POST['txt_fname']);
	$lname = strip_tags($_POST['txt_lname']);
	$email =strip_tags($_POST['txt_email']);


$sql = "update client set phone = '$phone',fax = '$fax', fname='$fname', lname='$lname', email = '$email' where client_id = '$clientId'"; 




if($conn->query($sql) === TRUE) {
$message = "Update Successful";
echo "<script type='text/javascript'>alert('Information Updated');window.location.href = 'workspace.php';</script>";


}



else
{
$message = "Update Failed";


	//echo $stmt->error;
}
}
	
	
?>


<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
  
    <title>Petlabs Diagnostic Laboratories Inc.</title>

    <link href="css/spacelab.min.css" rel="stylesheet">
    <link href="css/screen.css" rel="stylesheet">
	<style type="text/css">
	
		footer{ background:#333; color:#eee; font-size:11px; padding:20px;width:100%;}
		body{background-color:#F8F8F8; }
		.wrapper{min-height:100%; height:auto; !important; height:100%; margin:0 auto -155px;}
		.push { height:155px; }	
		.img-margin{margin-bottom:10px;}
		.active {font-weight:bold; color:#fff; border: 1px solid #FFFFFF;}
		.spacer{height:250px;}
		.registration{margin-left:600px;margin-top: 30px;}
		td, th{padding:5px;}
		.error{color:red; font-weight:bold; }
		.textbox-width{width:300px;}
	</style>


  </head>

  <body>
  <div class="wrapper">
  <header>
  <div class = "container">
  		  <div class="row">
    		  <div class="col-lg-6 col-md-6">
    		  	   <h4><a href="index.php" style="text-decoration: none;color:black;">Petlabs Diagnostics Laboratories Inc.</a></h4>
    		  </div>	
  		  </div>
  </div>
  
  
  
  </header>

<nav class="navbar navbar-default navbar-inverse ">
  <div class="container">
   
    <div class="navbar-header">
      <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#petlabs-navbar-collapse-1" aria-expanded="false">
        <span class="sr-only">Toggle navigation</span>
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
      </button>
    
    </div>


 
    <div class="collapse navbar-collapse" id="petlabs-navbar-collapse-1">
  
     
      <ul class="nav navbar-nav">
      	 <li><a href="workspace.php" >Workspace</a></li>
        <li><a href="client_admin.php">Admin</a></li>
         <li><a href="update_client.php" class="active">Update Info</a></li>
       <li><a href="register_hospital.php">Add Location</a></li>
    	  <li><a href="downloadFile.php?file=ClientHelp.pdf">Help</a></li>
    	  <li><a href="logout.inc.php">Logout</a></li>
    	    
      </ul>

    
    </div>
	
  </div><!-- container -->
</nav>


 
  
  
  <div class="container">
   	
    <div class="row">
              			<div class="col-lg-10 col-lg-offset-2">	
<form method="post" action="" id="plform" class="form-horizontal">


<fieldset>
 <p  style="font-weight:bold;">Update Information </p>
 <p class="error"><?php echo $message ?> </p>
 
 <hr/>
	
	<?php while($row = $result->fetch_assoc()){ ?>
	
<div class="form-group">
	
		<label for="txt_email" class="col-lg-2 control-label">Email: </label>
  <div class="col-lg-10">
		<input type="text" name="txt_email"  id="txt_email" class="col-xs-4" value="<?php echo $row['email']; ?>" />
</div>
	</div>

<div class="form-group">
	
		<label for="txt_fname" class="col-lg-2 control-label">First Name: </label>
  <div class="col-lg-10">
		<input type="text" name="txt_fname"  id="txt_fname" class="col-xs-4" value="<?php echo $row['fname']; ?>"  />
</div>
	</div>
<div class="form-group">
	
		<label for="txt_lname" class="col-lg-2 control-label">Last Name: </label>

  <div class="col-lg-10">
		<input type="text" name="txt_lname" id="txt_lname" class="col-xs-4" value="<?php echo $row['lname']; ?>"  />
</div>
	</div>	

	<div class="form-group">
	
		<label for="txt_phone" class="col-lg-2 control-label">Phone: </label>
  <div class="col-lg-10">
		<input type="text" name="txt_phone" id="txt_phone" value="<?php echo $row['phone']; ?>" />
</div>
	</div>
<div class="form-group">
	
		<label for="txt_fax" class="col-lg-2 control-label">Fax: </label>
  <div class="col-lg-10">
		<input type="text" name="txt_fax" id="txt_fax" minlength="10" value="<?php echo $row['fax']; ?>" />
</div>
	</div>


<?php

}

?>




<div class="form-group">
	 <div class="col-lg-10 col-lg-offset-2">

		 <button type="reset" class="btn btn-default">Cancel</button>
        	 <button type="submit" class="btn btn-primary" name="submit">Submit</button>
        	 </div>
</div>
</fieldset>
	</form>
</div>
</div>
</div>

</script>
 <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
    <!-- Include all compiled plugins (below), or include individual files as needed -->
    <script src="js/bootstrap.min.js"></script>



<script src="js/jquery.js"></script>
<script src="js/jquery.validate.js"></script>

 <script>


  $("#plform").validate({


            rules: {

               
                txt_phone: {
                    required: true,
                    minlength: 10
                },
                txt_fname: {
                    required: true,
                    minlength: 2
                },
                txt_lname: {
                    required: true,
                    minlength: 2
                },

                txt_email: {
                    required: true,                 
                    email:true
                }
                
                




            },

            messages: {
               
                txt_phone: {
                    required: "*",
                    minlength: "Minimum 10 characters"
                },
                txt_fname: {
                    required: "*",
                    minlength: "Minimum 2 characters"
                },
                txt_lname: {
                    required: "*",
                    minlength: "Minimum 2 characters"
                },
                txt_email: {
                    required: "*"
                   
                },
                
            }
        });
    </script>

   	
   	
   	
   	
   	</div>	
      





 	</div>	
   	   
<div class="spacer">  </div>
				<div class="push">
				</div>





<footer>
<div class="container">
	 <div class="row">
       <div class="col-lg-12 col-md-12">
       <p class="text-center">Petlabs Diagnostics Laboratories Inc. Main Office | 2510 Sub Station Road | Medina, Ohio 44256 | 330-220-6435</p>
	   <p class="text-center">Petlabs Animal Clinic Northview | 36400 Center Ridge Road | North Ridgeville, Ohio 44039 | 440-327-2062
        </div>
     </div>
	 
	 <div class="row">
	 <div class="col-lg-12 col-md-12">
	 <p class="text-center">Copyright &copy;2016 - Petlabs Diagnostics Laboratories Inc.</p>
	 </div>
	 </div>
</div> <!-- container-->
</footer>



</div><!-- wrapper -->
 <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
					    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
					    <!-- Include all compiled plugins (below), or include individual files as needed -->
					    <script src="js/bootstrap.min.js"></script>
  </body>
</html>
     